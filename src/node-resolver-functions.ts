...
import { getTsConfigDefaults, ComputeAsCommonRootOfFiles } from "./configuration";
...
class ProjectResolver {
  ...
  constuctor(
    ...
    const {rootDir, include: includeSpecs, files: filesArray, exclude: excludeSpecs} = getTsConfigDefaults(tsconfig, basePath, files, include, exclude);
    if(rootDir === ComputeAsCommonRootOfFiles) {
      throw new Error('Cannot determine rootDir if composite is not set.  Either enable composite or set explicit rootDir');
    }

    this.files = filesArray.map(f => normalizeSlashes(resolve(basePath, f)));
    const reString = ts.getRegularExpressionForWildcard(includeSpecs, basePath, 'files');
    this.includeRe = new RegExp(reString ?? '$^');
    const reString2 = ts.getRegularExpressionForWildcard(excludeSpecs as string[], basePath, 'exclude');
...
function createNodeResolverFunctions(opts: {
  ...
  function ensureTrailingSlash(str: string) {
    if(str.includes('\\')) throw new Error('String must have normalized slashes');
    if(!str.endsWith('/')) str += '/';
    return str;
  }
  ...
  const getAlternativeDirectoriesCached = cachedLookup(getAlternativeDirectories);
   ...
   */
  function* getAlternativeFilenames(filename: string, allowOmitFileExtension: boolean) {
    // TODO be sure to avoid .d.ts, .d.mts, and .d.cts
    const lastDotIndex = filename.lastIndexOf('.');
    let emittedReplacements = false;
    if(lastDotIndex > 0) {
      const endsInDts = filename.endsWith('.d.ts') || filename.endsWith('.d.cts') || filename.endsWith('.d.mts');
      if(!endsInDts) {
        const name = filename.slice(0, lastDotIndex);
        const extension = filename.slice(lastDotIndex + 1);
        const replacements = extension === 'js' ? jsExtensions : extension === 'cjs' ? cjsExtensions : extension === 'mjs' ? mjsExtensions : undefined;
        if(replacements) {
