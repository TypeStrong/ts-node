import type { CompilerHost, CompilerOptions, Diagnostic, SourceFile, TranspileOptions, TranspileOutput } from 'typescript';
import type { TSCommon } from './ts-compiler-types';

/** @internal */
export function createTsTranspileModule(ts: TSCommon, transpileOptions: Pick<TranspileOptions, 'compilerOptions' | 'reportDiagnostics' | 'transformers'>) {
  const {
    createProgram, createSourceFile, getDefaultCompilerOptions, getImpliedNodeFormatForFile, fixupCompilerOptions,
    transpileOptionValueCompilerOptions, getNewLineCharacter, fileExtensionIs, normalizePath, Debug, toPath,
    getSetExternalModuleIndicator, getEntries, addRange, hasProperty, getEmitScriptTarget, getDirectoryPath
  } = ts as any;

  const compilerOptionsDiagnostics: Diagnostic[] = [];

  const options: CompilerOptions = transpileOptions.compilerOptions ? fixupCompilerOptions(transpileOptions.compilerOptions, compilerOptionsDiagnostics) : {};
  ...
  function transpileModule(input: string, transpileOptions2: Pick<TranspileOptions, 'fileName' | 'moduleName' | 'renamedDependencies'>, packageJsonType: "module" | "commonjs" = "commonjs"): TranspileOutput {
    ...
    if (transpileOptions2.renamedDependencies) {
        (sourceFile as any).renamedDependencies = new Map(getEntries(transpileOptions2.renamedDependencies));
    }
